...
//string to hold user input to search bar
std::string searchInput = "";

//strings of first and second name entered for a street
std::string streetNameOne;
std::string streetNameTwo;
...
void draw_map() {
    ...
    if (mapLoadedOnce == false){
       ...
       // create_button("Change Map", "Help", act_on_help_button);
...
void act_on_button_press(float x, float y, t_event_buttonPressed event) {
    ...
    if (event.button == 1){ 
        ...
        ********************************************/
        ...
        /*The following commented and uncommented code 
         *is being used to debug our traveling saleseman 
         *algorthim
         
         *NOT PART OF GRAPHICS INTERFACE
         */
        
        ////////////////////////////////////////////////////////////////////////
        std::vector<DeliveryInfo> deliveries = {DeliveryInfo(44848, 98941), DeliveryInfo(108022, 62818), DeliveryInfo(17844, 76041)};
        std::vector<unsigned> depots = {80481, 34196, 47772};
        double turnPenalty = 15.0;
        //std::vector<unsigned> path = traveling_courier(deliveries, depots, turnPenalty);
        
        std::vector<unsigned> path = traveling_courier(deliveries, depots, turnPenalty);
        //for(unsigned i = 0; i < deliveries.size(); i++){
            //startIntersections[deliveries[i].pickUp] = true;
           // endIntersections[deliveries[i].dropOff] = true;
        //}
        
        //for(unsigned i = 0; i < depots.size(); i++){
            
            startIntersections[98941] = true;
            startIntersections[108022] = true;
       // }
        //StreetSegmentInfo segInfo = getStreetSegmentInfo(path[0]);
        /*
        std::cout<<"to: "<<segInfo.to<<std::endl;
        std::cout<<"from: "<<segInfo.from<<std::endl;
        std::cout<<"one way? "<<segInfo.oneWay<<std::endl;
        startIntersections[depots[0]] = true;
        */
        //startIntersections[75020] = true;
        //endIntersections[path[0]] == true;
        std::cout<<"pathsize: "<< path.size()<<std::endl;
        
        for(unsigned i = 521; i < 654; i++){
            
           segmentsToHighlight[path[i]] = true;
       ...
       /////////////////////////////////////////////////////////////////////////  
        ...
        int newX = xworld_to_scrn(x);
        int newY = yworld_to_scrn(y);

        // help box is clicked
        if (newX > 8 && newX < 68 && newY > 645 && newY < 700) {
            if(clickedHelp)
                clickedHelp = false;
            else
            clickedHelp = true;
...
void drawSearchBar(){
    ...
    draw_surface(load_png_from_file("/nfs/ug/homes-3/p/papajan6/ece297/work/mapper/libstreetmap/resources/help.png"), 8, 645);
...
void drawHelp(){
    ...
    setcolor(BUILDING);
    setlinestyle(SOLID);
    drawrect(50, 540 , 310, 815);
    setcolor(t_color(253, 248, 248, 170));
    fillrect(51, 539, 309, 814);
    setfontsize(10);

    // write out all individual strings to be sent to box
    string helpText1 = "Welcome to OurMap!";

    string helpText2 = "The following commands are available:";
    string helpText3 = "> Click 'Find' to search for an intersection or POI.";
    string helpText4 = "> Click 'Food', 'Health', 'Education',";
    string helpText5 = "or 'Entertainment'to filter Points of Interest.";
    string helpText6 = "> Click Change Map to view a new city";
    string helpText7 = "> Left click to find the nearest intersection ";
    string helpText8 = "> Right click two intersections to find directions";
    string helpText9 = "> Click 'Clear' to clear the map";
                 

    //create a box for each line of text
    t_bound_box helpTextBox1    = t_bound_box(50, 545, 310, 560);
    t_bound_box helpTextBox2    = t_bound_box(50, 560, 310, 575);
    t_bound_box helpTextBox3    = t_bound_box(50, 575, 310, 590);
    t_bound_box helpTextBox4    = t_bound_box(50, 590, 310, 605);
    t_bound_box helpTextBox5    = t_bound_box(50, 605, 310, 620);
    t_bound_box helpTextBox6    = t_bound_box(50, 620, 310, 635);
    t_bound_box helpTextBox7    = t_bound_box(50, 635, 310, 650);
    t_bound_box helpTextBox8    = t_bound_box(50, 650, 310, 665);
    t_bound_box helpTextBox9    = t_bound_box(50, 665, 310, 680);

    // write out each line of text, alternate colours for headings and body text
    setcolor(BLACK);
    drawtext_in(helpTextBox1, helpText1);
    setcolor(t_color(102, 102, 102));
    drawtext_in(helpTextBox2, helpText2);
    drawtext_in(helpTextBox3, helpText3);
    drawtext_in(helpTextBox4, helpText4);
    drawtext_in(helpTextBox5, helpText5);
    drawtext_in(helpTextBox6, helpText6);
    drawtext_in(helpTextBox7, helpText7);
    drawtext_in(helpTextBox8, helpText8);
    drawtext_in(helpTextBox9, helpText9);
